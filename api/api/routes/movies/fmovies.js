...
const routes = async (fastify, options) => {
    ...
    fastify.get('/:query', async (request, reply) => {
        ...
        let res = main_1.redis
            ? await cache_1.default.fetch(main_1.redis, `fmovies:${query}:${page}`, async () => await fmovies.search(query, page ? page : 1), 60 * 60 * 6)
    ...
    fastify.get('/info', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `fmovies:info:${id}`, async () => await fmovies.fetchMediaInfo(id), 60 * 60 * 3)
    ...
    fastify.get('/watch', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `fmovies:watch:${episodeId}:${mediaId}:${server}`, async () => await fmovies.fetchEpisodeSources(episodeId, mediaId, server), 60 * 30)
