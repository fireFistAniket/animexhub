...
class M3U8Proxy {
    ...
    getM3U8Proxy = async (fastify, options) => {
        fastify.get('/m3u8-proxy/*', async (request, reply) => {
            // split params
            ...
            // queries to object
            const data = await this.getM3U8(decodedUrl.startsWith('https')
                ? decodedUrl
        ...
        fastify.get('/m3u8/*', async (request, reply) => {
            const params = request.params['*'];
            var url = Buffer.from(params, 'base64').toString('utf8');
            try {
                var req = await axios_1.default.get(url, {
                    headers: {
