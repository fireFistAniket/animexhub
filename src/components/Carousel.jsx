    ...
    <div className='w-full overflow-hidden'>
      <div className='relative min-h-[45vh] md:min-h-[65vh] xl:min-h-[80vh]'>
        <AnimatePresence initial={false} custom={direction}>
          ...
          >
            {!slides[currentIndex]?.attributes.coverImage && (
              <img
                src={
                  (currentIndex === 0 &&
                    "/animexhub/static-carousel-img-1.jpg") ||
                  (currentIndex === 1 && "/animexhub/static-carousel-img-2.jpg")
                }
                alt='anime-cover'
            ...
            {slides[currentIndex]?.attributes.coverImage && (
              <div className='absolute w-full bottom-0 bg-black bg-opacity-30 px-[3vmax] py-[1vmin] backdrop-blur flex flex-col items-start gap-[1.2vmin]'>
                ...
                <p className='text-[1.5vmax] flex items-center gap-2 font-medium text-neutral-100'>
                  Rating:{" "}
                  {Math.round(
                    parseInt(slides[currentIndex]?.attributes?.averageRating) /
                      10
                  )}
                  <FaStar color='#fece3c' />
                </p>
                <div className='flex items-center gap-[3vmin]'>
                  ...
                  <p className='bg-teal-600 rounded-md text-[1.2vmax] text-[--primary] font-semibold px-[2vmin] py-[0.5vmin] shadow shadow-neutral-600'>
                    {slides[currentIndex]?.attributes?.showType}
                  </p>
                </div>
              </div>
            )}
